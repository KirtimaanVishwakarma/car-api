class ApiFeatures {
  ...
  filter() {
    ...
    const removeFields = ["keyword", "page", "size", "limit"];

    const valuesToCheck = Object.keys(queryCopy);
    const anyValueMatches = valuesToCheck.some((value) =>
      removeFields.includes(value)
    );
    if (anyValueMatches) {
